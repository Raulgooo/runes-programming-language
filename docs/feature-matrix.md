...
| Generic functions/types/variants/methods | `monomorphize.c` | generic codegen/error tests, imported `Option<TextSplit>` specialization, nested generic buffered adapters satisfying generated interface constraints | [Generics](reference/semantics.md#generics) | Complete for root/private and cross-module public concrete arguments; private nested-module arguments pending |
| Associated methods and type-qualified calls | `parser.c`, `monomorphize.c`, `typecheck.c`, `codegen.c` | ordinary/generic owners, aliases, method generics, misuse diagnostics, realm matrix, generated-C and sanitizer checks | [Methods](reference/syntax.md#interfaces-and-methods) | Complete for direct calls; function-value extraction and interface requirements pending |
...
| Implicit effective-realm specialization of direct `flex` calls | `monomorphize.c`, `codegen.c` | flex specialization/allocation and associated-method realm tests | [Flex functions](reference/memory-and-unsafe.md#flex-functions) | Complete for direct free and associated functions; erased function values pending |
...
| Typed realm-sensitive backing storage | `monomorphize.c`, `typecheck.c`, `codegen.c`, `runtime.c`, `std/allocation.runes` | ordinary and recoverable `t` allocation, initialized publication, pointer-bearing shrink, overflow/failure/owner runtime tests, generated-C and sanitizer coverage | [Allocation](reference/allocation.md#typed-storage-and-recoverable-t-operations) | Complete |
| Realm-aware `Vec<T>` | `std/vec.runes`, realm/type specialization and storage lowering | ordinary and recoverable `t` APIs, dynamic/regional/GC generic-helper specialization, failure injection, pointer-bearing forced GC, nested-owner rejection, ASan/UBSan/leak checks | [`Vec<T>` reference](reference/vec.md) | Complete initial API and contract; move-only enforcement pending |
| Borrowed UTF-8 `std.text` | `std/text.runes`, runtime UTF-8 primitives, imported-generic local-type specialization | normal/edge behavior, scalar cursor, checked boundaries, embedded NUL, generated-C no-allocation check, UTF-8 properties, ASan/UBSan, fuzz seed | [Borrowed text](reference/text.md) | Complete initial allocation-free API |
| Realm-aware owning `String` | `std/string.runes`, `Vec<u8>`, runtime UTF-8 view/validation, non-generic realm-associated resolution and specialized extern/type dependency preservation | all UTF-8 widths, invalid encodings, embedded NUL, checked boundaries, allocation/publication failure, dynamic/regional/GC, stack rejection, generated C, ASan/UBSan/leaks, fuzz seed | [`String` reference](reference/string.md) | Complete initial API and contract; move-only enforcement pending |
...
| Core, bytes, borrowed/owning text, vector, formatting, parsing, static I/O, lexical paths, and owning hosted files | `src/std/` | result/bytes/text/String/Vec/format/parse/I/O suites, real standard-stream integration, path lexical/realm/failure coverage, and real/fake filesystem, descriptor, realm, strict-C, and sanitizer coverage | [Standard library](reference/standard-library.md), [I/O](reference/io.md), [Paths](reference/path.md), [Filesystem](reference/fs.md) | Complete for documented M0-M7 surface |
